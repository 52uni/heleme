...
| 版本 | 内容 |
|------|------|
| v1.1 | 检查更新按钮移到侧边栏底部 + 自动更新功能（Gitee + GitHub 双源 fallback） |
...
- tag 格式: `v1.1`, `v1.2`, `v2.0` 等
- versionCode = 主版本号 * 100 + 次版本号（v1.0 = 100, v1.1 = 101, v2.0 = 200）
- versionName 与 tag 保持一致（不含 v 前缀）
...
```powershell
git add -A .
git commit -m "vX.X - 更新说明"
git tag vX.X
git push origin master --tags
git push gitee master --tags
```

#### 3. 创建 GitHub Release 并上传 APK（Unicode 转义方式）

```powershell
# PowerShell 下发送中文 JSON 需使用 [System.Text.Encoding]::UTF8.GetBytes()
$token = "github_pat_..."
$json = '{"tag_name":"v1.2","name":"v1.2 - ...","body":"..."}'
$bodyBytes = [System.Text.Encoding]::UTF8.GetBytes($json)
$release = Invoke-RestMethod `
    -Uri "https://api.github.com/repos/52uni/heleme/releases" `
    -Method Post `
...
# 上传 APK
$uploadUrl = "https://uploads.github.com/repos/52uni/heleme/releases/$($release.id)/assets?name=WaterReminder-v$version.apk"
Invoke-RestMethod -Uri $uploadUrl -Method Post `
    -Headers @{ Authorization = "Bearer $token"; Accept = "application/vnd.github+json" } `
    -ContentType "application/vnd.android.package-archive" `
    -InFile "app\build\outputs\apk\debug\app-debug.apk"
```

#### 4. 创建 Gitee Release 并上传 APK

```powershell
# Gitee 创建 Release（同 GitHub，body 中中文用 \uXXXX Unicode 转义即可）
$token = "0cd76..."
$json = '{"access_token":"...","tag_name":"v1.2","name":"v1.2 - ...","body":"...","target_commitish":"master"}'
$bodyBytes = [System.Text.Encoding]::UTF8.GetBytes($json)
$release = Invoke-RestMethod `
    -Uri "https://gitee.com/api/v5/repos/gui_1124/heleme/releases" `
    -Method Post `
    -Body $bodyBytes `
    -ContentType "application/json;charset=utf-8"

# 上传 APK（Gitee 使用 attach_files API + WebClient）
$wc = New-Object System.Net.WebClient
$wc.UploadFile(
    "https://gitee.com/api/v5/repos/gui_1124/heleme/releases/$($release.id)/attach_files?access_token=$token",
...
App 启动或点击"检查更新"时：
1. 优先请求 Gitee API（国内快）
2. Gitee 失败则 fallback GitHub API
3. 比对返回的 versionCode 与本地 BuildConfig.VERSION_CODE
4. 有新版本 → 弹窗 → 点击下载 → DownloadManager 下载 → 通知进度 → 系统安装器安装

### 关键文件

| 文件 | 作用 |
|------|------|
| `app/src/main/java/com/zhengui/waterreminder/util/UpdateManager.kt` | 检查更新 + 下载 + 安装逻辑 |
| `app/src/main/AndroidManifest.xml` | REQUEST_INSTALL_PACKAGES 权限 + FileProvider |
| `app/src/main/res/xml/file_paths.xml` | APK 下载缓存路径 |
